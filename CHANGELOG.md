...
### Changed
- **No functional change on Cursor.** This release changes how the
  must-display hook surfaces a workflow's progress marker, and Cursor does
  not ship that hook — on Cursor the marker has always been rendered by the
  assistant's own reply, which is exactly the behaviour this release moves
  the other platforms toward. The version is bumped to keep Forge's plugin
  releases aligned across platforms.
